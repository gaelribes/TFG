...
hidden_dimensions = [150, 140, 130, 120, 110, 100, 90, 80, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15]
latent_dim = 4
dropout_range = [dropout for dropout in np.arange(0,0.25,0.01)]
dropout_layers_range = [lay for lay in range(0,10)]
learning_rate_range = [lr for lr in np.arange(0.0001, 0.0005, 0.00025)]  # Example range for learning rate

best_validation_loss = float('inf')
best_hyperparameters = None

# Perform grid search
for dropout, dropout_layers, learning_rate in itertools.product(dropout_range, dropout_layers_range, learning_rate_range):
    ...
    for epoch in range(num_epochs):
        ...
        validation_losses.append(evaluation_loss.item())

    # Compute average validation loss
    average_validation_loss = sum(validation_losses) / len(validation_losses)
    
    # Print hyperparameters and validation loss
    print(f'Hyperparameters: Hidden Dimensions={hidden_dimensions}, Latent Dim={latent_dim}, Dropout={dropout}, Dropout Layers={dropout_layers}, Learning Rate={learning_rate}, Validation Loss={average_validation_loss}')

    # Update best hyperparameters if current combination has lower validation loss
    if average_validation_loss < best_validation_loss:
        best_validation_loss = average_validation_loss
        best_hyperparameters = (hidden_dimensions, latent_dim, dropout, dropout_layers, learning_rate)

print(f'Best hyperparameters: Hidden Dimensions={best_hyperparameters[0]}, Latent Dim={best_hyperparameters[1]}, Dropout={best_hyperparameters[2]}, Dropout Layers={best_hyperparameters[3]}, Learning Rate={best_hyperparameters[4]}, Validation Loss={best_validation_loss}')
